...
- `getPayloadType` → `String::class.java` (raw JSON 수신)
- `handleFrame` → `log.info("[STOMP] Received metric: {}", payload)` 로그 출력
...
[STOMP] Received metric: {"id":1,"metricType":"CPU_USAGE","value":42.35,"unit":"%","recordedAt":"2026-03-09T..."}
```
...
```
[STOMP] Received metric: {"metricType":"CPU_USAGE","value":XX.XX,...}
```
